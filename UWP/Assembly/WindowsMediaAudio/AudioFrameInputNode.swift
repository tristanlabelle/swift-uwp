...
public final class WindowsMediaAudio_AudioFrameInputNode: WindowsRuntime.WinRTImport<WindowsMediaAudio_AudioFrameInputNodeProjection>, WindowsMediaAudio_IAudioInputNodeProtocol, WindowsMediaAudio_IAudioNodeProtocol, WindowsFoundation_IClosableProtocol, WindowsMediaAudio_IAudioInputNode2Protocol {
    // MARK: Windows.Media.Audio.IAudioFrameInputNode members
    ...
    /// - Returns: A value indicating the queued sample count.
    public var queuedSampleCount: Swift.UInt64 {
        get throws {
            try _interop.get_QueuedSampleCount()
        }
    }

    /// Notifies of a completed audio frame that has been submitted to the graph with a call to AddFrame.
    public func audioFrameCompleted(adding handler: WindowsFoundation_TypedEventHandler<WindowsMediaAudio_AudioFrameInputNode?, WindowsMediaAudio_AudioFrameCompletedEventArgs?>?) throws -> WindowsRuntime.EventRegistration {
        let _token = try _interop.add_AudioFrameCompleted(handler)
        return WindowsRuntime.EventRegistration(token: _token, remover: audioFrameCompleted)
    }

    public func audioFrameCompleted(removing token: WindowsRuntime.EventRegistrationToken) throws {
        try _interop.remove_AudioFrameCompleted(token)
    }

    /// Raised when the audio graph is ready to begin processing a new quantum of data.
    public func quantumStarted(adding handler: WindowsFoundation_TypedEventHandler<WindowsMediaAudio_AudioFrameInputNode?, WindowsMediaAudio_FrameInputNodeQuantumStartedEventArgs?>?) throws -> WindowsRuntime.EventRegistration {
    ...
    /// - Parameter frame: An AudioFrame object representing the frame.
    public func addFrame(_ frame: WindowsMedia_AudioFrame?) throws {
        try _interop.addFrame(frame)
    }

    /// Removes queued frames from the audio frame input node.
    public func discardQueuedFrames() throws {
        try _interop.discardQueuedFrames()
    }
    ...
    /// - Returns: True if the audio frame input node consumes input, and false otherwise.
    ...
    /// - Returns: The spatial audio emitter associated with the AudioFrameInputNode.
